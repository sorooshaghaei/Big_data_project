...
# legacy local-file pipeline kept only for archive and comparison
...
from .legacy_context import enrich_daily_with_context
from .legacy_io import clean_numeric, combine_grouped_frames, iter_table_smart, parse_any_date, read_table_smart
from .pipeline import PipelineArtifacts

FACT_GROUP_COLS = ["date", "region", "location_id", "location_name", "metric_type", "source"]

# limits sample mode to the first chunk
def _chunk_limit(sample_mode: bool) -> int | None:
    return 1 if sample_mode else None

# pulls paris travel-title counts into one table
def _load_travel_titles_fact(paths: ProjectPaths, sample_mode: bool) -> pd.DataFrame:
    ...
    chunk_limit = _chunk_limit(sample_mode)
    ...
    for chunk_idx, chunk in enumerate(iter_table_smart(paths.travel_titles, chunksize=chunksize, usecols=usecols)):
        ...
        if chunk_limit is not None and chunk_idx + 1 >= chunk_limit:
            break

    return combine_grouped_frames(partials, FACT_GROUP_COLS, ["value"])

# rolls up the idfm surface file by day and line
...
# gathers the ile de france rail files
...
# turns the mta hourly file into daily and hourly tables
def _load_mta_fact_and_hourly_profile(paths: ProjectPaths, sample_mode: bool) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    chunksize = 250_000 if sample_mode else 1_000_000
    chunk_limit = _chunk_limit(sample_mode)
...
# collects paris hourly share files
...
# combines all source tables into one station table
...
# turns station rows into daily totals
...
# writes the generated tables to csv files
...
# runs the local file pipeline end to end
